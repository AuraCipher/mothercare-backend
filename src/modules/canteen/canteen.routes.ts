...
router.post('/products', asyncHandler(async (req, res) => {
  const { categoryId, supplierId, name, unitPrice, boxPrice, unitsPerBox, stockBoxes, stockUnits, lowStockThreshold } = req.body;
  ...
  const data = await canteenService.createProduct(
    getCanteenBranchId(req),
    { categoryId, supplierId, name, unitPrice, boxPrice, unitsPerBox, stockBoxes, stockUnits, lowStockThreshold },
    getCanteenUserId(req),
  );
  res.status(201).json({ success: true, data });
}));
